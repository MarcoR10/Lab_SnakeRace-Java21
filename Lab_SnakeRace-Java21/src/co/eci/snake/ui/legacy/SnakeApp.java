...
public final class SnakeApp extends JFrame {
    ...
    //--------------------------------------------------------------------------//
    // Punto 3: Iniciar/Pausar/Reanudar con estadísticas consistentes (sin tearing).
    //--------------------------------------------------------------------------//
    private void togglePause() {
        if ("Pausar".equals(actionButton.getText())) {
            // pauseMovement() de cada runner espera (internamente) a que su paso en curso
            // termine antes de retornar, así que al salir de este forEach ninguna serpiente
            // se sigue moviendo y el estado (posiciones/longitudes) es estable para leerlo.
            runners.forEach(SnakeRunner::pauseMovement);
            clock.pause();
            showConsistentStats();
            actionButton.setText("Reanudar");
        } else {
            statsLabel.setText(" ");
            runners.forEach(SnakeRunner::resumeMovement);
            clock.resume();
            actionButton.setText("Pausar");
        }
    }

    /** Calcula, sobre el estado ya congelado (todas las serpientes detenidas), la serpiente
     *  viva más larga y la primera en morir, y las muestra en la etiqueta superior. */
    private void showConsistentStats() {
        Snake longestAlive = null;
        Snake firstDead = null;
        for (Snake s : snakes) {
            if (s.isAlive()) {
                if (longestAlive == null || s.length() > longestAlive.length()) longestAlive = s;
            } else if (firstDead == null || s.deathOrder() < firstDead.deathOrder()) {
                firstDead = s;
            }
        }
        String longestTxt = (longestAlive != null)
                ? "Más larga viva: serpiente #" + snakes.indexOf(longestAlive) + " (" + longestAlive.length() + ")"
                : "No quedan serpientes vivas";
        String worstTxt = (firstDead != null)
                ? "Peor serpiente (murió primero): #" + snakes.indexOf(firstDead)
                : "Ninguna serpiente ha muerto todavía";
        statsLabel.setText(longestTxt + "     |     " + worstTxt);
    }
